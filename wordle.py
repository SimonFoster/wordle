...
# answers = [ a for a in answers if a.startswith('aG_') ]

if __name__ == '__main__':
    while len(answers) > 1:
        t = make_tree( guesses, answers)
        # dump_tree(t)
        count, guess = find_best_guess( t )
        print( 'Best guess is ', guess, ' and there are no more than', count, ' words left' )
        score = input( 'Please enter the score: eg "GY_GY_": ')
        # print( guess,score )
        answers = t[guess][score]
        print( answers )
